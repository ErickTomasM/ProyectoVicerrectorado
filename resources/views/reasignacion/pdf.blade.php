    ...
    <div class="page_break">
        ...
        Presente <br>
          ...
          @endforeach 
            ...
            nos permitimos comunicarle que en su condición de  
            ...
            <strong>Tiempo Completo </strong>
               ...
               <strong> Tiempo horario </strong>
                @endif
            de la carrera de  <strong class="capitalize">{{ ucwords(strtolower($designacion->carrera->programa)) }}</strong>

            en concordancia con el Articulo 11 del Reglamento del Régimen Académico Docente de la 
            Universidad Boliviana, ha sido 
            @if($isReasignacion)
            <strong>Reasignado</strong> <!-- Cambiar el texto a "Reasignado" -->
        @else
            <strong>REASIGNADO</strong> <!-- Mantener el texto como "Designado" -->
        @endif
            docente por
            ...
            <table style="margin: 0 auto;">
              ...
              <tbody>
                ...
                @foreach($docente->asignaciones as $asignacion)
                    ...
                    @endphp
                    <tr>
                        <td style="text-align: left;">{{ $asignacion->materia->sigla }}</td>
                        <td style="text-align: left;">
                            {{ $asignacion->materia->materia }}
                        </td>
                        <td style="text-align: center;">{{ $asignacion->grupo }}</td>
                        <td style="text-align: right;">{{ $horas }}Hrs.</td>
          ...
          @foreach($facultad as $facu)
            <div class="left">
              ...
              <text class="capitalize">{{$facu->decano->abre_titulo}} {{ucwords(strtolower($facu->decano->nombres))}} {{ucwords(strtolower($facu->decano->paterno))}} {{ucwords(strtolower($facu->decano->materno))}}</text> <br>
                Decano de la Facultad de <text>{{ ucwords(strtolower($facu->facultad)) }}</text> <br>
              @else
              <text class="capitalize">{{$facu->decano->abre_titulo}} {{ucwords(strtolower($facu->decano->nombres))}} {{ucwords(strtolower($facu->decano->paterno))}} {{ucwords(strtolower($facu->decano->materno))}}</text> <br>
